...
def test_ask_success(monkeypatch):
    class FakeCollection:
        def query(self, query_texts, n_results):
            return {
                "documents": [[
                    "Node CPU usage is 45%.",
                    "Node memory usage is 60%."
                ]]
            }
    ...
    class FakeResponse:
        ...
        def json(self):
            return {
                "response": "The Kubernetes node is operating normally."
            }

    monkeypatch.setattr(
        main.chromadb,
        "HttpClient",
        lambda host, port: FakeChromaClient()
    )
    ...
    response = client.post(
        "/ask",
        json={
            "question": "What is the current node status?"
        }
    )
    ...
    assert body["answer"] == (
        "The Kubernetes node is operating normally."
    )
...
def test_ask_ollama_service_failure(monkeypatch):
    ...
    assert response.status_code == 503
    assert "Ollama service request failed" in (
